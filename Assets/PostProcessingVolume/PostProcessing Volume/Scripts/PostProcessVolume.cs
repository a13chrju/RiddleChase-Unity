...
public class PostProcessVolume : MonoBehaviour
{
    ...
    void Start () {
        ...
        PostProcessVolumeReceiver receivers = this.GetComponent<PostProcessVolumeReceiver>();
        if (receivers != null)
        {
            var thisVolume = this.GetComponent<PostProcessVolume>();
            receivers.SetValues(ref thisVolume, GradientPercentage(this.transform.position));
        }
    }
